...
SELECT 
    VALS,
    TRANSLATE(VALS,'0123456789','XXXXXXXXXX') AS Replaced
FROM GetIntegers;
...
SELECT m.Name AS Manager, COUNT(e.EmployeeID) AS NumEmployees
FROM EmployeesManager e
JOIN EmployeesManager m ON e.ManagerID = m.EmployeeID
GROUP BY m.Name
HAVING COUNT(e.EmployeeID) > 3;

SELECT EmployeeName, Salary
FROM EmployeesSalary
WHERE Salary > (SELECT AVG(Salary) FROM EmployeesSalary);
...
WHERE NOT EXISTS (
    ...
    WHERE NOT EXISTS (
        ...
        WHERE m.StudentID = s.StudentID
          AND m.SubjectID = sub.SubjectID
    )
);


SELECT e.Name AS Employee, e.Salary, m.Name AS Manager, m.Salary AS ManagerSalary
FROM EmployeesManager2 e
JOIN EmployeesManager2 m ON e.ManagerID = m.EmployeeID
...
FROM EmployeesSalaryDept e
WHERE e.Salary > (
    SELECT AVG(Salary)
    FROM EmployeesSalaryDept
    WHERE Department = e.Department
)
AND e.Salary < (
    SELECT AVG(Salary)
...
SELECT e.EmployeeName, e.Department, e.Salary, e.HireDate
FROM EmployeesHire e
WHERE e.Salary > (
    SELECT AVG(Salary)
    FROM EmployeesHire
    WHERE Department = e.Department
