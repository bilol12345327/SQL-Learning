...
SELECT CAST(emp_id AS VARCHAR) + '-' + first_name + ' ' + last_name AS FullInfo
FROM employees
WHERE emp_id = 100;

SELECT REPLACE(phone_number, '124', '999') AS NewPhone
FROM employees;
...
SELECT year,
       (SELECT MAX(v)
        FROM (VALUES (Max1), (Max2), (Max3)) AS Value(v)) AS BiggestValue
FROM TestMax;
...
SELECT COALESCE(col1, col2, col3, col4) AS FirstNotNull
FROM person;
...
SELECT o1.customer_id, o1.state AS CaliforniaOrder, o2.state AS TexasOrder
FROM Orders o1
JOIN Orders o2 ON o1.customer_id = o2.customer_id
WHERE o1.state = 'California' AND o2.state = 'Texas';

SELECT STRING_AGG(Value, ', ') AS AllValues
FROM DMLTable;
...
DECLARE @txt VARCHAR(100) = 'tf56sd#%OqH';
DECLARE @i INT = 1;
DECLARE @ch CHAR(1);

DECLARE @UpperCase VARCHAR(100) = '';
DECLARE @LowerCase VARCHAR(100) = '';
DECLARE @Digits VARCHAR(100) = '';
DECLARE @Others VARCHAR(100) = '';

WHILE @i <= LEN(@txt)
BEGIN
    SET @ch = SUBSTRING(@txt, @i, 1);

    IF ASCII(@ch) BETWEEN 65 AND 90      -- A-Z
        SET @UpperCase += @ch;
    ELSE IF ASCII(@ch) BETWEEN 97 AND 122 -- a-z
        SET @LowerCase += @ch;
    ELSE IF ASCII(@ch) BETWEEN 48 AND 57  -- 0-9
        SET @Digits += @ch;
    ELSE
        SET @Others += @ch;
...
SELECT StudentID,
       Score,
       SUM(Score) OVER (ORDER BY StudentID ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS RunningTotal
FROM Students;

SELECT s1.StudentID, s1.FullName, s1.BirthDate
FROM Student s1
JOIN Student s2 
...
SELECT 
    ...
    ROUND(AVG(CAST(c2.amount AS FLOAT)), 2) AS average_amount
FROM (
