...
INSERT INTO Saled(SaleID, ProductID, CustomerID, SaleDate, SaleAmount) VALUES
...
SELECT YEAR(OrderDate) AS OrderYear,
       ...
       CASE 
           WHEN AVG(TotalAmount) < 200 THEN 'Low Avg'
           WHEN AVG(TotalAmount) < 500 THEN 'Medium Avg'
           ELSE 'High Avg'
       END AS AvgGroup
FROM Orders
...
SELECT ProductName,
       CASE 
            WHEN ProductName LIKE '%oo%' THEN 'Contains oo'
            ELSE 'Does not contain oo'
       END AS CheckName
FROM Products;
...
SELECT CustomerID, SUM(TotalAmount) AS TotalSpent,
       CASE 
            WHEN SUM(TotalAmount) > 10000 THEN 'VIP'
            ELSE 'Regular'
       END AS CustomerType
FROM Invoices
GROUP BY CustomerID
...
SELECT ProductID, COUNT(SaleID) AS TimesSold,
       CASE 
            WHEN COUNT(SaleID) = 0 THEN 'Never Sold'
            WHEN COUNT(SaleID) < 5 THEN 'Rarely Sold'
            ELSE 'Popular'
       END AS SaleStatus
FROM Saled
GROUP BY ProductID;
